  ...
  carregarProdutos: (lista, idcategoria) => {
    if (lista.length > 0) {
      ...
      lista.forEach((e, i) => {
        ...
        let preco = parseFloat(e.valor); // Converte para número
        let precoFormatado = !isNaN(preco)
  ...
  carregarOpcionaisProdutoSimples: (lista) => {
    ...
    if (listaSimples.length > 0) {
      ...
      listaSimples.forEach((e, i) => {
        ...
        if (
          !isNaN(parseFloat(e.valoropcional)) &&
          parseFloat(e.valoropcional) > 0
        ) {
          valor = `+ R$ ${parseFloat(e.valoropcional)
            .toFixed(2)
            .replace(".", ",")}`;
        }
